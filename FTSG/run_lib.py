...
def write_Data2(step,training_loss):
    filedir="loss.txt"
    with open(osp.join('./train_loss/',filedir),"a+") as f:#a+
        f.writelines('['+str(round(step, 2))+' '+str(round(training_loss, 4))+']')
        f.write('\n')
...
#  inverse_scaler = datasets.get_data_inverse_scaler(config)
  scaler = datasets.no_get_data_scaler(config)
  inverse_scaler = datasets.no_get_data_inverse_scaler(config)
  # Setup SDEs
...
#        sample = np.clip(sample.permute(0, 2, 3, 1).cpu().numpy() * 255, 0, 255).astype(np.uint8)
        ...
        '''
        sample = sample.permute(0, 2, 3, 1).cpu().numpy()
        kw_real, kw_imag = (sample[0,:,:,0]+sample[0,:,:,2])/2, sample[0,:,:,1]
...
#        kw_real, kw_imag = (samples[0,:,:,0]+samples[0,:,:,2])/2, samples[0,:,:,1]
#        kw_complex = kw_real+1j*kw_imag
        kw_real = samples[0,:,:,0]
        kw_complex = kw_real
