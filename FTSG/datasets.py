...
class GetMRI(Dataset):
    def __init__(self,root,augment=None):
        self.files = np.array([root+"/"+x  for x in os.listdir(root)])
        self.augment = None
    ...
    def __getitem__(self,index):
        ...
        img_k = np.expand_dims(img,axis=0)
        return img_k   
...
def no_get_data_inverse_scaler(config):
  """Inverse data normalizer."""
  if config.data.centered:
    # Rescale [-1, 1] to [0, 1]
    return lambda x: x
...
def get_dataset(config, uniform_dequantization=False, evaluation=False):
  ...
  dataset = GetMRI(root= "./datasets_all")
